...
<section class="ml-0 lg:ml-[250px] p-8">
    ...
    @endif
    <a href="/profil-{{ $name }}" class="flex items-center mb-10 text-lg font-semibold transition-all duration-300 ease-in-out text-primary hover:text-sky-800">
    ...
    <a href="/anggota-{{ $name }}/{{ $slug }}/create" class="px-6 py-3 text-sm font-semibold text-white transition duration-300 ease-in-out rounded-md bg-primary hover:shadow-lg hover:opacity-50">+ Tambah Anggota</a>
            ...
            <table class="min-w-full mb-0">
                ...
                <tbody>
                    ...
                    <tr class="bg-gray-100 border-b">
                        ...
                        <td class="px-4 py-2 whitespace-nowrap"><img src="{{ asset('storage/' . $member->gambar_anggota) }}" alt="{{ $member->gambar_anggota }}" class="max-w-[120px]"></td>
                        <td class="px-4 py-2 text-sm font-normal text-left text-gray-500 whitespace-nowrap">{{ $member->nama_anggota }}</td>
                        <td class="px-4 py-2 text-sm font-normal text-left text-gray-500 whitespace-nowrap">{{ $member->kelas }}</td>
                        <td class="px-4 py-2 text-sm font-normal text-left text-gray-500 whitespace-nowrap">{{ $member->instagram_anggota }}</td>
                        <td class="px-4 py-2 text-sm font-normal text-right whitespace-nowrap">
                            <a href="/anggota-{{ $name }}/{{ $slug }}/{{ $member->id }}/edit" class="font-medium text-blue-600 transition duration-300 ease-in-out hover:text-blue-700 focus:text-blue-700 active:text-blue-800">Edit</a>
                            <form action="/anggota-{{ $name }}/{{ $slug }}/{{ $member->id }}" method="post" class="inline-block">
                                @method('delete')
                                @csrf
