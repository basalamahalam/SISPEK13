...
<section class="ml-0 lg:ml-[250px] p-8">
    ...
    <h2 class="text-sky-800 font-small text-lg mb-10">Input Ekskul</h2>
    <form action="/ekskul-admin" method="POST" enctype="multipart/form-data" class="max-w mx-auto">
        @csrf
        <div class="mb-6">
            <label for="name" class="text-sky-800 text-lg font-semibold block mb-2">Nama Ekskul</label>
            <input type="text" class="bg-gray-300 rounded-md block w-1/2 p-2 @error('name') border-red-500 @enderror" id="name" name="name" value="{{ old('name') }}" required autofocus>
            @error('name')
        ...
        <div class="mb-6">
            ...
            <input type="text" class="bg-gray-300 rounded-md block w-1/2 p-2 @error('link') border-red-500 @enderror" id="link" name="link" value="{{ old('link') }}" required autofocus>
            @error('link')
                <p class="text-red-500 mt-2">{{ $message }}</p>
            @enderror
        </div>
        <div class="mb-6">
            <label for="image" class="text-sky-800 text-lg font-semibold block">Logo Ekskul</label>
