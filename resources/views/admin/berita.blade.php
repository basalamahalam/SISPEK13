...
<section class="ml-0 lg:ml-[250px] p-8">
    ...
    <a href="/berita-admin/create" class="px-6 py-3 text-sm font-semibold text-white transition duration-300 ease-in-out rounded-md bg-primary hover:shadow-lg hover:opacity-50">+ Tambah Berita</a>
    <div class="flex flex-col mt-10 mb-10 overflow-x-auto bg-white rounded-lg shadow-lg">
        <div class="inline-block min-w-full overflow-hidden">
                ...
                <tbody>
                    @foreach($data as $berita)
                        <tr class="bg-gray-100 border-b">
                            ...
                            <td class="px-4 py-2 text-sm font-normal text-left text-gray-500 whitespace-nowrap">{{ Str::limit($berita->title, $limit = 15, $end = '...') }}</td>
                            <td class="px-4 py-2 text-sm text-gray-500 whitespace-nowrap fomt-normal">{!! Str::limit($berita->body, $limit = 50, $end = '...') !!}</td>
                            <td class="px-4 py-2 text-sm font-normal text-left text-gray-500 whitespace-nowrap">{{  Str::limit($berita->created_at, $limit = 10, $end = '...') }}</td>
                            <td class="px-4 py-2 text-sm font-normal text-right whitespace-nowrap">
                                <a href="/berita-admin/{{ $berita->slug }}/edit" class="font-medium text-blue-600 transition duration-300 ease-in-out hover:text-blue-700 focus:text-blue-700 active:text-blue-800">Edit</a>
            ...
            @else
            <p class="py-3 text-center text-gray-500">Berita masih kosong</p>
            @endif
        </div>
    </div>
</section>