...
<section class="ml-0 lg:ml-[250px] p-8">
    ...
    <a href="/jurnal-admin/create" class="text-sm font-semibold text-white bg-primary py-3 px-6 rounded-md hover:shadow-lg hover:opacity-50 transition duration-300 ease-in-out">+ Tambah Jurnal</a>
        ...
        <div class="inline-block min-w-full overflow-hidden">
            ...
            <table class="min-w-full mb-0">
                ...
                <tr>
                    ...
                    <th scope="col" class="text-sm font-medium px-4 py-2">Penulis</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b bg-gray-100">
                        ...
                        <td class="px-4 py-2 whitespace-nowrap text-sm fomt-normal text-gray-500">{{ Str::limit($jurnal->title, $limit = 20, $end = '...') }}</td>
                        <td class="text-sm font-normal px-4 py-2 whitespace-nowrap text-left text-gray-500">{!! Str::limit($jurnal->body, $limit = 30, $end = '...') !!}</td>
                        <td class="text-sm font-normal px-4 py-2 whitespace-nowrap text-left text-gray-500">{{ Str::limit($jurnal->author, $limit = 15, $end = '...') }}</td>
                        <td class="text-sm font-normal px-4 py-2 whitespace-nowrap text-left text-gray-500">{{  Str::limit($jurnal->created_at, $limit = 10, $end = '...') }}</td>
                        <td class="text-sm font-normal px-4 py-2 whitespace-nowrap text-right">
                            <a href="/jurnal-admin/{{ $jurnal->slug }}/edit" class="font-medium text-blue-600 hover:text-blue-700 focus:text-blue-700 active:text-blue-800 transition duration-300 ease-in-out">Edit</a>
            ...
            @else
                <p class="text-gray-500 text-center py-3">Jurnal masih kosong</p>
            @endif
        </div>
        {{ $data->links() }}
    </div>
