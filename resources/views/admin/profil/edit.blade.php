...
<section class="ml-0 lg:ml-[250px] p-8">
    ...
    <form action="/profil-{{ $name }}/{{ $profil->id }}" method="POST" enctype="multipart/form-data" class="max-w mx-auto">
        @method('put')
        @csrf
        <input type="hidden" class="bg-gray-300 rounded-md block w-1/2 p-2 @error('nama_angkatan') border-red-500 @enderror" id="nama_organisasi" name="nama_organisasi" value="{{ $profil->nama_organisasi }}">
        <div class="mb-6">
            <label for="nama_angkatan" class="text-sky-800 text-lg font-semibold block mb-2">Nama Angkatan</label>
            <input type="text" class="bg-gray-300 rounded-md block w-1/2 p-2 @error('nama_angkatan') border-red-500 @enderror" id="nama_angkatan" name="nama_angkatan" value="{{ old('nama_angkatan', $profil->nama_angkatan) }}" required autofocus>
            @error('nama_angkatan')
       ...
       <div class="mb-6">
            ...
            <input type="hidden" name="oldImage" value="{{ $profil->gambar_angkatan }}">
            @if ($profil->gambar_angkatan)
                <img src="{{ asset('storage/' . $profil->gambar_angkatan) }}" class="img-preview max-w-sm mb-3 col-sm-5 d-block">
            @else
            ...
            <input type="file" class="block w-1/2 border cursor-pointer bg-gray-300 border-gray-200 shadow-sm rounded-lg text-sm focus:z-10
                ...
                file:py-3 file:px-4 @error('gambar_angkatan') border-red-500 @enderror" id="gambar_angkatan" name="gambar_angkatan" onchange="previewImage()">
        ...
        <div class="mb-10">
            ...
            @error('visi_angkatan')
                <p class="text-red-500 mt-2">{{ $message }}</p>
            @enderror
            <input id="visi_angkatan" type="hidden" name="visi_angkatan" value="{{ old('visi_angkatan', $profil->visi_angkatan) }}" required>
        ...
        <div class="mb-10">
            ...
            @error('misi_angkatan')
                <p class="text-red-500 mt-2">{{ $message }}</p>
            @enderror
            <input id="misi_angkatan" type="hidden" name="misi_angkatan" value="{{ old('misi_angkatan', $profil->misi_angkatan) }}" required>
