...
<section class="ml-0 lg:ml-[250px] p-8">
    ...
    <form action="/event/{{ $slug }}" method="POST" enctype="multipart/form-data" class="max-w mx-auto">
        @csrf
        ...
        <div class="mb-6">
            ...
            <input type="text" class="bg-gray-300 rounded-md block w-1/2 p-2 @error('nama_acara') border-red-500 @enderror" id="nama_acara" name="nama_acara" value="{{ old('nama_acara') }}" required autofocus>
            @error('nama_acara')
        ...
        <div class="mb-6">
            ...
            <input type="date" class="bg-gray-300 rounded-md block w-1/2 p-2 @error('tanggal_acara') border-red-500 @enderror" id="tanggal_acara" name="tanggal_acara" value="{{ old('tanggal_acara') }}" required autofocus>
            @error('tanggal_acara')
                <p class="text-red-500 mt-2">{{ $message }}</p>
            @enderror
        </div>
        <div class="mb-10">
            ...
            @error('deskripsi_acara')
                <p class="text-red-500 mt-2">{{ $message }}</p>
            @enderror
            <input id="deskripsi_acara" type="hidden" name="deskripsi_acara" value="{{ old('deskripsi_acara') }}" required>
